#include<bits/stdc++.h>
using namespace std;
int listsize=1,currentpostion=1;
class mytodo{
public:
    string description;
    string duedate;
...
bool addtodolist(string description,string duedate,int priority){
    if(listsize<100){
        mytodolist[listsize].description=description;
        mytodolist[listsize].duedate=duedate;
        mytodolist[listsize].priority=priority;
        listsize++;
        return true;
    }
    else{
...
bool showtodolist(){
    for(int i=currentpostion;i<listsize;i++){
